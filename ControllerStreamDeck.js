...
const log = debug('sio:streamdeck')
...
export class ControllerStreamDeck extends EventEmitter {
  ...
  async connect(options = {}) {
    ...
    if (streamDecks[0].model !== this._model || streamDecks[0].serialNumber !== this._serialNumber) {
      this.log('warn', 'The stream deck is not configured correctly!')
  ...
  async send(options) {
    ...
    console.log('sending message to stream deck', options)
    try {
      ...
      console.log(`error while setting image on stream deck button ${keyIndex} with label ${label}`, error)
    }
  }
