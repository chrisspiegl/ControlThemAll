...
export class ControlThemAll {
   ...
   */
	...
	midiOnNoteOn(message) {
		...
		const {note, velocity: value, channel} = message;
	...
	midiOnControllerChange(message) {
		...
		const {controller: note, value, channel} = message;
	...
	getButtonAction(name) {
		const buttonActions = {
			...
			Delay: (options, value) => delay(options.duration),

			KeyboardFire: (options, value) => this.controllerKeyboard.fire(options.combination),

			KeyboardPress: (options, value) => this.controllerKeyboard.pressKey(options.combination),

			KeyboardRelease: (options, value) => this.controllerKeyboard.releaseKey(options.combination),

			KeyboardType: (options, value) => this.controllerKeyboard.type(options.text),

			KeyboardTypeViaClipboard: (options, value) => this.controllerKeyboard.typeViaClipboard(options.text),

			KeyboardTypeViaKeyboard: (options, value) => this.controllerKeyboard.typeViaKeyboard(options.text),
